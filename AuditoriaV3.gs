...
function audV3PromptSistemaSdr_() {
  return [
    ...
    'Erros prováveis de transcrição automática em nomes próprios, nomes de colaboradores ou marcas não constituem desvio de execução quando o contexto identifica claramente a mesma entidade. Exemplos: Eline/Elane para Elaine ou variações fonéticas de Hitecnet. Preserve a fala original como evidência, mas normalize a entidade apenas para fins de comparação com o pitch. Nunca aplique essa tolerância a números, valores, datas, horários, métricas, condições comerciais ou termos que alterem o sentido da fala.',
